<?php
    ...
    if(isset($_FILES['file'])){
        ...
        $final_name = "users/cover_imgs/" . $uid . "." . "jpg";
        ...
        if($file_size > 8388608){
            $errors[] = 'File size cannot exceed 8 MB';
            $error_msg = 'File size cannot exceed 8 MB';
            $response = array('status' => "failed", "msg" => "Upload Failed.", "error" => $error_msg);
            echo json_encode($response);
        }     
        ...
        if(empty($errors) == true){
            ...
            if (move_uploaded_file($file_tmp, "../" . $final_name)) {
                ...
                $query = "UPDATE users SET cover_img = '$final_name' WHERE id = '$uid'";
